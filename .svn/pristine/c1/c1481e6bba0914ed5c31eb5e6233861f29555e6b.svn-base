...
namespace HB.ApiModel.auto.messages
{
    public class AvailabilityRQ : AbstractGenericRequest
    {
        ...
        public void Validate()
        {
            ...
            for (var i = 0; i < occupancies.Count; i++)
            {
                ...
                if (occupancies[i].children.HasValue)
                {
                    ...
                    for (var p = 0; p < occupancies[i].paxes.Count; p++)
                    {
            ...
            if (destination != null)
            {
                if (string.IsNullOrEmpty(destination.code) || !destination.zone.HasValue)
                    throw new ArgumentException(
                        "If destination object is informed then code and zone must be informed too.");
            }
            ...
            if (geolocation != null)
            {
                if (!geolocation.longitude.HasValue || !geolocation.latitude.HasValue || !geolocation.radius.HasValue)
                    throw new ArgumentException(
                        "If geolocation object is informed then longitude and latitude and radius must be informed too.");
            }
            ...
            if (hotels != null)
            {
                if (hotels.hotel != null && hotels.hotel.Count == 0)
                    throw new ArgumentException(
                        "If hotelsFilter object is informed then internal hotel code must be informed too.");
            }
            ...
            if (boards != null)
            {
                if (boards.board == null || boards.board.Count == 0)
                    throw new ArgumentException(
                        "If boards object is informed then internal board list code must be informed too.");

                for (var b = 0; b < boards.board.Count; b++)
                {
