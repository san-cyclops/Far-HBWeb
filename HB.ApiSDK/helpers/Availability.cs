...
namespace HB.ApiSDK.helpers
{
    public class Availability
    {
        ...
        public AvailabilityRQ toAvailabilityRQ()
        {
            try
            {
                var availabilityRQ = new AvailabilityRQ();
                availabilityRQ.language = language;
                ...
                {
                    availabilityRQ.keywords = new KeywordsFilter(matchingKeywords.ToList(),
                        keywordsMatcher.Equals(Matcher.ALL)); // matchingKeywords.ToList();                       
                }

                if (includeHotels != null && includeHotels.Count > 0 && excludeHotels != null && excludeHotels.Count > 0)
                    foreach (var e in excludeHotels)
                        includeHotels.RemoveAll(i => i == e);

                if (includeHotels != null && includeHotels.Count > 0)
                {
                    var hotelsFilter = new HotelsFilter();
                    hotelsFilter.included = true;
                    hotelsFilter.hotel = includeHotels;
                    hotelsFilter.type = useGiataCodes
                        ? SimpleTypes.HotelCodeType.GIATA
                        : SimpleTypes.HotelCodeType.HOTELBEDS;
                ...
                var reviewsFilter = new List<ReviewFilter>();
                ...
                if (tripAdvisorScoreHigherThan != null || tripAdvisorScoreLowerThan != null ||
                    numberOfTripAdvisorReviewsHigherThan != null)
                {
                    var reviewFilter = new ReviewFilter();
                ...
                if (limitHotelsTo != null || maxCategory != null || minCategory != null || limitRoomsPerHotelTo != null ||
                    limitRatesPerRoomTo != null || ratesLowerThan != null
