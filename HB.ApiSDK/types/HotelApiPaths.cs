...
namespace HB.ApiSDK.types
{
    public class HotelApiPaths
    {
        public class HotelApiPathsBase
        {
            ...
            {
                return urlTemplate.Replace("${path}", basePath)
                    .Replace("${version}", version.version == HotelApiVersion.versions.V0_2 ? "0.2" : "1.0");
            }
        ...
        public class BOOKING_LIST : HotelApiPathsBase
        {
            public BOOKING_LIST()
            {
                httpMethod = new HttpMethod("GET");
                endpoint =
                    "bookings?start=${start}&end=${end}&filterType=${filterType}&includeCancelled=${includeCancelled}&from=${from}&to=${to}";
            }
        }
    }
}