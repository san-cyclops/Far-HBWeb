...
app.service('studentService', function ($http) {
    this.get = function (HotelCodex, Destinationx, DepartureDatex, ReturnDatex, Roomsx, Adults1x, Children1x, C1Age1x, C1Age2x, C1Age3x, C1Age4x, Adults2x, Children2x, C2Age1x, C2Age2x, C2Age3x, C2Age4x, Adults3x, Children3x, C3Age1x, C3Age2x, C3Age3x, C3Age4x, visitorsCurrencyx, Markupx) {
        ...
        var url = "/api/HotelBooking/GetHotelDetailsRS?HotelCode=" + HotelCodex + "&des=" + Destinationx + "&DepartureDate=" + DepartureDatex + "&ReturnDate=" + ReturnDatex + "&Rooms=" + Roomsx + "&Adults1=" + Adults1x + "&Children1=" + Children1x + "&C1Age1=" + C1Age1x + "&C1Age2=" + C1Age2x + "&C1Age3=" + C1Age3x + "&C1Age4=" + C1Age4x + "&Adults2=" + Adults2x + "&Children2=" + Children2x + "&C2Age1=" + C2Age1x + "&C2Age2=" + C2Age2x + "&C2Age3=" + C2Age3x + "&C2Age4=" + C2Age4x + "&Adults3=" + Adults3x + "&Children3=" + Children3x + "&C3Age1=" + C3Age1x + "&C3Age2=" + C3Age2x + "&C3Age3=" + C3Age3x + "&C3Age4=" + C3Age4x + "&visitorsCurrency=" + visitorsCurrencyx + "&Markup=" + Markupx;
        var response = $http.get(url);
        return response;
    };
    this.getratecommnet = function (rateKey, Rooms, Adults1, Children1, C1Age1, C1Age2, C1Age3, C1Age4, Adults2, Children2, C2Age1, C2Age2, C2Age3, C2Age4, Adults3, Children3, C3Age1, C3Age2, C3Age3, C3Age4, visitorsCurrency , Markup) {
        // var url2 = "http://hotels.tag2travel.com:4585/api/HotelBooking/GetCheckRateRs?rateKey=" + sub;
        var url2 = "/api/HotelBooking/GetCheckRateRs?rateKey="+rateKey+ "&Rooms=" + Rooms+ "&Adults1=" +Adults1 +"&Children1=" +Children1 + "&C1Age1=" +C1Age1 + "&C1Age2=" +C1Age2 + "&C1Age3=" +C1Age3 + "&C1Age4=" +C1Age4+  "&Adults2="+Adults2+ "&Children2=" +Children2 + "&C2Age1=" +C2Age1 + "&C2Age2=" +C2Age2 + "&C2Age3=" +C2Age3 + "&C2Age4=" +C2Age4 +  "&Adults3=" +Adults3 + "&Children3=" +Children3 + "&C3Age1=" +C3Age1 + "&C3Age2=" +C3Age2 + "&C3Age3=" +C3Age3 + "&C3Age4=" +C3Age4 + "&visitorsCurrency=" +visitorsCurrency + "&Markup="+ Markup;                                                                
...
app.controller('ctrl', function ($scope, $q, $window, $http, studentService, filterFilter) {
    ...
    $scope.visitorsCurrency = $scope.datax.visitorsCurrency;
    $scope.Name = "";
    $scope.HotelName = "";
    $scope.HotelAddress = "";
    $scope.HotelPhone = "";
    ...
    loadData($scope.datax.HotelCode, $scope.datax.Des, $scope.datax.DepartureDate, $scope.datax.ReturnDate, $scope.datax.Rooms, $scope.datax.Adults1, $scope.datax.Children1, $scope.datax.C1Age1, $scope.datax.C1Age2, $scope.datax.C1Age3, $scope.datax.C1Age4, $scope.datax.Adults2, $scope.datax.Children2, $scope.datax.C2Age1, $scope.datax.C2Age2, $scope.datax.C2Age3, $scope.datax.C2Age4, $scope.datax.Adults3, $scope.datax.Children3, $scope.datax.C3Age1, $scope.datax.C3Age2, $scope.datax.C3Age3, $scope.datax.C3Age4, $scope.datax.visitorsCurrency, $scope.datax.Markup);

    function loadData(HotelCodex, Destinationx, DepartureDatex, ReturnDatex, Roomsx, Adults1x, Children1x, C1Age1x, C1Age2x, C1Age3x, C1Age4x, Adults2x, Children2x, C2Age1x, C2Age2x, C2Age3x, C2Age4x, Adults3x, Children3x, C3Age1x, C3Age2x, C3Age3x, C3Age4x,visitorsCurrencyx , Markupx) {
        $scope.Markup = Markupx;
        var promiseStudent = studentService.get(HotelCodex, Destinationx, DepartureDatex, ReturnDatex, Roomsx, Adults1x, Children1x, C1Age1x, C1Age2x, C1Age3x, C1Age4x, Adults2x, Children2x, C2Age1x, C2Age2x, C2Age3x, C2Age4x, Adults3x, Children3x, C3Age1x, C3Age2x, C3Age3x, C3Age4x, visitorsCurrencyx , Markupx);

        $scope.combineResult = $q.all([
                promiseStudent
        ]).then(function (resp) {
            console.log("sdfuisdfhsdfguagsdfiubafsd");
   
            $scope.hotels = angular.fromJson(resp[0].data);


            $scope.respData = $scope.hotels[0];

         
            $scope.rooms = $scope.hotels[0].rooms;
            $scope.facilities =  $scope.hotels[0].facilityrep;
            $scope.DateFromx = $scope.hotels[0].checkIn;
            $scope.DateTox =  $scope.hotels[0].checkOut;
            $scope.loaded = true;
            $scope.HotelName = $scope.hotels[0].name;
            $scope.HotelAddress =  $scope.hotels[0].address;
            $scope.HotelPhone =  $scope.hotels[0].phoneNumber;
            $scope.Destination =  $scope.hotels[0].destinationName;
            $scope.Board =  $scope.hotels[0].categoryName;
            $scope.Description =  $scope.hotels[0].description;
            $scope.lat = parseFloat($scope.hotels[0].latitude);
            $scope.lng = parseFloat($scope.hotels[0].longitude);
            $scope.AccommodationType = $scope.hotels[0].accommodationTypeCode;
            $scope.InterestPoint =  $scope.hotels[0].interestPoints[0];
            $scope.postalCode = $scope.hotels[0].postalCode;
            $scope.hotelEmail = $scope.hotels[0].email;
           //$scope.InterestPointObj = [
            // {
    ...
    $scope.removeTask = function (taskId) {
        ...
        var removeTask = studentService.getratecommnet($scope.rateKey, $scope.datax.Rooms, $scope.datax.Adults1, $scope.datax.Children1, $scope.datax.C1Age1, $scope.datax.C1Age2, $scope.datax.C1Age3, $scope.datax.C1Age4, $scope.datax.Adults2, $scope.datax.Children2, $scope.datax.C2Age1, $scope.datax.C2Age2, $scope.datax.C2Age3, $scope.datax.C2Age4, $scope.datax.Adults3, $scope.datax.Children3, $scope.datax.C3Age1, $scope.datax.C3Age2, $scope.datax.C3Age3, $scope.datax.C3Age4, $scope.datax.visitorsCurrency, $scope.Markup);
        removeTask.then(function (d) {
         ...
         // $scope.respData = d;s
           ...
           // $scope.PriceOther = $scope.rate.hotel.rooms[0].rates[0].visitorsRate;// other price sends price from visitors currency

            $scope.NetPrice = (($scope.Price * 3 / 100) + $scope.Price).toFixed(2);
           // $scope.NetPriceOther = (($scope.PriceOther * 3 / 100) + $scope.PriceOther).toFixed(2);

           
            $scope.rateCurrency = $scope.rate.hotel.rooms[0].strRateCurrency;
   ...
   { name: 'Ms', }
    ...
    $scope.sendList = function () {
        ...
        for (var i = 0; i < $scope.questionelemnt.length; i++) {
            ...
            if (i === 2) {
                $scope.Adults3 = $scope.questionelemnt[2].name;
        ...
        var paymentData = {
            ...
            hoteladdress: $scope.HotelAddress + $scope.postalCode,
            hotelphone: $scope.HotelPhone,
            hotelemail: $scope.email,
